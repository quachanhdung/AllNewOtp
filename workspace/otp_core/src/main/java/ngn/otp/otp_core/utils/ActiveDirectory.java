...
public class ActiveDirectory {
	private static final Logger logger = LogManager.getLogger(ActiveDirectory.class);
   ...
   */
    public static LdapContext getConnection(String username, String password, String domainName, String serverURL) throws NamingException {
...
//        String ldapURL = "ldap://" + serverName;
        ...
        logger.info("=====ActiveDirectory authen=====");
        logger.info("server: "+ldapURL);
...
//                NamingEnumeration<SearchResult> answer = context.search( toDC(domainName), "(& (sAMAccountName="+principalName+")(objectClass=user))", controls);
//                if (answer.hasMore()) {
...
//    }
    ...
    public static User getUser(String username, LdapContext context, String searchBase, String searchFilter) {
        try{
        	 ...
        	 if (username.contains("@")){
                ...
                controls.setSearchScope(SUBTREE_SCOPE);
                controls.setReturningAttributes(userAttributes);	
           	 	String filter = "(& (sAMAccountName="+principalName+")"+searchFilter+")";
           	 	System.out.println(filter);
                NamingEnumeration<SearchResult> answer = context.search( searchBase, filter, controls);
                if (answer.hasMore()) {
                    Attributes attr = answer.next().getAttributes();
                    Attribute user = attr.get("userPrincipalName");
                    //test
   ...
   */
    public static User[] getUsers(LdapContext context, String searchBase, String filter) throws NamingException {
...
//            System.out.println(toDC(domainName));
//            NamingEnumeration answer = context.search( toDC(domainName), searchBase, controls);
            NamingEnumeration answer = context.search( searchBase, filter, controls);
            try{
   ...
   */
    ...
    private static String getErrorCode(String s) {
    	String result = s;
    	try {
			...
			if(result.equalsIgnoreCase("52e")) {
				return "invalid credentials";
			}
			
			if(result.equalsIgnoreCase("525")) {
				return "user not found";
			}
			
			if(result.equalsIgnoreCase("530")) {
				return "not permitted to logon at this time";
			}	
			
			if(result.equalsIgnoreCase("532")) {
				return "password expired";
			}	
			
			if(result.equalsIgnoreCase("533")) {
				return "account disabled";
			}
			
			if(result.equalsIgnoreCase("701")) {
				return "account expired";
			}
			
			if(result.equalsIgnoreCase("773")) {
				return "user must reset password";
			}
			
			if(result.equalsIgnoreCase("775")) {
